...
async def async_stream_call():
    ...
    # 因为astream 是一个异步生成器函数，调用时会返回一个异步生成器对象，而不是直接返回结果，所以不需要使用 await 来等待结果，而是直接得到一个可以异步迭代的生成器对象
    ...
    # 异步遍历异步生成器（必须使用 async for，不可用普通 for）
    # 异步遍历异步生成器（必须使用 async for，不可用普通 for）
    # 因为异步for在每一步都会进行await操作，等待下一个数据块的到来，适合处理流式数据
    # 异步生成器里，每次 yield 一个值出来都是一次网络请求，都需要等待，所以必须使用 async for 来遍历，才能正确处理每次网络请求的结果
